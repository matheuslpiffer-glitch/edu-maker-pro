...
import { Send, FileDown, Loader2, Image as ImageIcon, FileText } from 'lucide-react';
...
export function MatAvatar({ size = 'md', className }) {
    ...
    return (<div className={cn(dim, 'shrink-0 flex items-center justify-center', className)}>
      <img src={src} alt="Mat" className="w-full h-auto object-contain drop-shadow-[0px_3px_6px_rgba(0,0,0,0.12)]"/>
    </div>);
}
    ...
    return (<div className={cn('flex flex-col min-h-0 flex-1 bg-white', className)}>
      <div ref={scrollRef} className={cn('flex-1 overflow-y-auto bg-white scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent', fullPage ? 'px-4 sm:px-8 py-8 space-y-8' : 'px-6 py-8 space-y-8')}>
        <div className={cn(fullPage ? 'mx-auto w-full max-w-3xl space-y-8' : 'space-y-8')}>
          {messages.map((msg, i) => (<div key={i} className={cn('flex gap-4 max-w-[90%]', msg.role === 'user' ? 'ml-auto flex-row-reverse' : 'mr-auto')}>
              ...
              <div className={cn('text-sm leading-relaxed px-1 py-1', msg.role === 'user' ? 'bg-slate-50 rounded-2xl px-4 py-3' : 'text-slate-700 w-full')}>
                {msg.role === 'assistant' ? (<div className="prose prose-sm prose-slate max-w-none prose-p:leading-relaxed prose-pre:bg-slate-900 prose-pre:text-slate-50">
                    ...
                    {msg.content.length > 100 && (<div className="mt-4 flex flex-wrap gap-2 no-print">
                        <button onClick={() => downloadAsPdf(msg.content)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
      ...
      <div className={cn('bg-white border-t border-slate-100', fullPage ? 'px-4 sm:px-8 py-4' : 'px-6 py-4')}>
        <div className={cn(fullPage && 'mx-auto w-full max-w-3xl')}>
          ...
          <div className="relative flex items-end gap-2 bg-slate-50 rounded-2xl border border-slate-200 focus-within:border-slate-300 focus-within:ring-1 focus-within:ring-slate-300 transition-all p-2">
            ...
            <button onClick={() => fileInputRef.current?.click()} disabled={isLoading || isUploading} className="mb-1 h-8 w-8 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 flex items-center justify-center transition-colors shrink-0" title="Subir arquivo (PDF, DOCX, TXT) ou foto (PNG, JPG)">
