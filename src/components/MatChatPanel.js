...
import { Send, FileUp, FileDown, Loader2, Image as ImageIcon, FileSpreadsheet, Presentation } from 'lucide-react';
...
export function renderMathAsUnicode(text) {
...
export function MatAvatar({ size = 'md', className }) {
    ...
    return (<div className={cn(dim, 'shrink-0 flex items-center justify-center', className)}>
      <img src={src} alt="Mat" className="w-full h-auto object-contain filter drop-shadow-[0px_3px_6px_rgba(0,0,0,0.12)]"/>
    </div>);
}
export default function MatChatPanel({ fullPage = false, onRegisterReset, className }) {
    ...
    const handleFileUpload = async (e) => {
        ...
        if (!file)
            return;
        setIsUploading(true);
        const userMsg = { role: 'user', content: `📎 Arquivo enviado: ${file.name}` };
    ...
    return (<div className={cn('flex flex-col min-h-0 flex-1 bg-white', className)}>
      <div ref={scrollRef} className={cn('flex-1 overflow-y-auto bg-white scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent', fullPage ? 'px-4 sm:px-8 py-8 space-y-8' : 'px-6 py-8 space-y-8')}>
        <div className={cn(fullPage ? 'mx-auto w-full max-w-3xl space-y-8' : 'space-y-8')}>
          {messages.map((msg, i) => (<div key={i} className={cn('flex flex-col gap-2 max-w-[90%]', msg.role === 'user' ? 'ml-auto items-end' : 'mr-auto items-start')}>
              <div className={cn('flex gap-4 w-full', msg.role === 'user' ? 'flex-row-reverse' : '')}>
                ...
                <div className={cn('text-sm leading-relaxed px-1 py-1', msg.role === 'user' ? 'bg-slate-50 rounded-2xl px-4 py-3 border border-slate-100' : 'text-slate-700 w-full')}>
                  {msg.role === 'assistant' ? (<div className="prose prose-sm prose-slate max-w-none prose-p:leading-relaxed prose-pre:bg-slate-900 prose-pre:text-slate-50">
              ...
              {msg.role === 'assistant' && msg.content.length > 50 && (<div className="flex flex-wrap gap-2 mt-1 ml-12 no-print">
                  <button onClick={() => downloadAsPdf(msg.content)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 text-[10px] font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-900 hover:border-slate-300 transition-all shadow-sm bg-white">
                    <FileDown className="h-3.5 w-3.5 text-blue-500"/>
                    BAIXAR PDF / DOCX
                  </button>
                  
                  {(msg.content.includes('|') || msg.content.includes('<table>')) && (<button onClick={() => {
          ...
          {isLoading && messages[messages.length - 1]?.role === 'user' && (<div className="flex gap-4">
              <MatAvatar size="sm"/>
      ...
      <div className={cn('bg-white border-t border-slate-100', fullPage ? 'px-4 sm:px-8 py-4' : 'px-6 py-4')}>
        <div className={cn(fullPage && 'mx-auto w-full max-w-3xl')}>
          {/* Quick Action Chips */}
          {!isStudentMode && (<div className="flex flex-wrap gap-2 mb-4 overflow-x-auto pb-1 no-scrollbar">
              {[
            ...
            ].map((chip) => (<button key={chip.label} onClick={() => {
                    setInput(chip.text);
                    inputRef.current?.focus();
          ...
          <div className="relative flex items-end gap-2 bg-slate-50 rounded-2xl border border-slate-200 focus-within:border-slate-300 focus-within:ring-1 focus-within:ring-slate-300 transition-all p-2">
            <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} accept=".pdf,.docx,.txt,.png,.jpg,.jpeg,.webp"/>
            <button onClick={() => {
            if (fileInputRef.current) {
                fileInputRef.current.accept = ".png,.jpg,.jpeg,.webp";
                fileInputRef.current.click();
            }
        }} disabled={isLoading || isUploading} className="mb-1 h-8 w-8 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 flex items-center justify-center transition-colors shrink-0" title="Subir foto (PNG, JPG)">
        ...
        }} disabled={isLoading || isUploading} className="mb-1 h-8 w-8 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 flex items-center justify-center transition-colors shrink-0" title="Subir documento (PDF, DOCX, TXT, CSV, XLSX)">
              <FileUp className="h-5 w-5"/>
            </button>
              {isUploading && <Loader2 className="h-4 w-4 animate-spin text-slate-400 mb-2 ml-1"/>}
            <textarea ref={inputRef} rows={1} value={input} onChange={e => {
        ...
        }} placeholder="Pergunte ao Mat..." className="flex-1 bg-transparent border-0 outline-none text-sm text-slate-800 placeholder:text-slate-400 px-3 py-2.5 resize-none min-h-[40px] max-h-[160px]"/>
            <button onClick={sendMessage} disabled={!input.trim() || isLoading} className="mb-1 h-8 w-8 rounded-lg bg-slate-900 text-white flex items-center justify-center hover:bg-slate-800 transition-colors disabled:opacity-20 disabled:cursor-not-allowed shrink-0" aria-label="Enviar mensagem">
