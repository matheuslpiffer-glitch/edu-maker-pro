  ...
  try {
    ...
    if (isInclusao) {
      const diretrizesPorPerfil: Record<string, string> = {
        ...
        aee_toc: `Forneça CRITÉRIOS DE CONCLUSÃO claros e objetivos em cada questão (o aluno precisa saber exatamente quando a tarefa está terminada, para reduzir a checagem repetitiva). EVITE comandos que sugiram perfeição ou refazer (não use "capriche", "sem nenhum erro", "revise quantas vezes precisar"). Use enunciados PREVISÍVEIS e padronizados, com a mesma estrutura em todas as questões. Reduza a pressão de tempo. Quando houver passos, deixe a lista FINITA e numerada (1, 2, 3 — fim), sem etapas abertas. Tom tranquilizador e direto.`,
        aee_tag: `Use tom ACOLHEDOR e de BAIXO RISCO (enfatize que é treino/aprendizado, não cobrança). Quebre a atividade em ETAPAS PEQUENAS e visíveis, uma de cada vez, para reduzir a sobrecarga. Dê instruções CLARAS e completas, sem surpresas ou pegadinhas. EVITE linguagem de pressão ("rápido", "valendo nota", "só tem uma chance"). Inclua mensagens curtas de encorajamento (ex.: "✅ Vá com calma, você consegue"). Estrutura previsível e espaçada, sem excesso de estímulos.`,
        aee_tpac: `Priorize a informação ESCRITA e VISUAL sobre a oral — todo conteúdo essencial deve estar no texto/imagem, nunca dependente de explicação falada. Use frases CURTAS e diretas. Apoie palavras-chave com pictogramas/emojis (ex.: "energia ⚡", "som 🔊"). EVITE depender de rima, sonoridade, ditado ou comandos apenas auditivos. Cada instrução deve ser compreensível só pela LEITURA. DESTAQUE verbos de comando em <strong>negrito</strong>.`,
        aee_tdl: `Reduza a CARGA LINGUÍSTICA: UMA ideia por frase, vocabulário CONCRETO e cotidiano, frases curtas na ordem direta (sujeito–verbo–objeto). EVITE orações subordinadas longas, dupla negativa, sinônimos raros e linguagem figurada. Apoie cada conceito-chave com imagem/emoji (ex.: "casa 🏠", "comer 🍎"). Prefira perguntas objetivas e alternativas curtas. Reformule enunciados longos em uma sequência de frases simples.`,
        aee_sensorial: `Cuide da CARGA SENSORIAL do material: layout LIMPO e organizado, SEM poluição visual (evite excesso de cores fortes, fundos carregados e elementos competindo). Use espaçamento generoso e poucos destaques por bloco. Mantenha a apresentação PREVISÍVEL e consistente em todas as questões. EVITE estímulos intensos. Prefira tons suaves e contraste confortável. Instruções curtas e calmas, permitindo processar um item de cada vez.`,
